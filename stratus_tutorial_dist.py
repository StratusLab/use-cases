cat > stratus_tutorial_dist.py <<'EOF'
#!/usr/bin/env python
...
class Tutorial:
    ...
    def _setFieldsFromEnvVars(self):
        ...
        self._setSingleFieldFromEnvVar('ubuntu', 'UBUNTU_IMAGE')
    ...
    def stratusRunInstance(self, image, persistentDisk=None):
        cmd = ["stratus-run-instance", "--quiet", "--key", self.sshKeyPub, image]
        if persistentDisk:
    ...
    def stratusAttachVolume(self, vmId, uuid):
        return self._execute(["stratus-attach-volume", "-i", vmId, uuid])

    def stratusDetachVolume(self, vmId, uuid):
        return self._execute(["stratus-detach-volume", "-i", vmId, uuid])
    ...
    def sshcmd(self, vmip):
        self._execute(['ssh', 'root@%s' % vmip.strip(), 'modprobe', 'acpiphp'])
    ...
    def run(self):
        ...
        imgMarketPlaceUri = self.stratusUploadMetadata(self.marketplace, self.fakeMetadata)
        ...
        #########
        self.printAction("Persistent disk storage Hot plug test")
        #########
        
        self.printStep("Running an Ubuntu VM")
        vm_id_ip = self.stratusRunInstance(self.ubuntu)
        ...
        self.printStep("Attaching persistent disk to VM")
        self.stratusAttachVolume(self.vmId,uuid)

        self.printStep("Detaching persistent disk to VM")
        self.stratusDetachVolume(self.vmId,uuid)
